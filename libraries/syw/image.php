	...
	*/
	...
	{			
		...
		if ($thumbnail == false) {
			...
			if ($this->isTransparent()) { // only works with .gif!
				// Get the transparent color values for the current image
			...
			switch ($this->image_mimetype) {
				...
				case 'image/png': 
					if ($handle_high_resolution) {
						...
						// keep transparency
						$alpha_channel = imagecolorallocatealpha($this->thumbnail, 0, 0, 0, 127);
						imagecolortransparent($this->thumbnail, $alpha_channel);
						imagefill($this->thumbnail, 0, 0, $alpha_channel);						
	...
	public function __destruct()
	{
		...
		if ($this->thumbnail_high_res != null) {
