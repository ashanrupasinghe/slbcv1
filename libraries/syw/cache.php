...
// 			}
	 ...
	 * If not, creates the proper folders, adding index.html files inside them
	 *
	 */
	public static function isFolderReady($root_path, $extra_path) 
	{		
	...
	public static function getTmpPath($tmp_path_param = 'default', $image_sub_directory = 'thumbnails') 
	{
		...
		if ($tmp_path_param == 'images') {
			...
			if (self::isFolderReady(JPATH_ROOT.'/'.$images_path, $image_sub_directory)) {
				$tmp_path = $images_path.'/'.$image_sub_directory;
			} else {
				$lang = JFactory::getLanguage();
				$lang->load('lib_syw.sys', JPATH_SITE);
				$app->enqueueMessage(JText::sprintf('LIB_SYW_WARNING_COULDNOTCREATETMPFOLDERUSINGDEFAULT', $images_path.'/'.$image_sub_directory), 'warning');
			}
		}
